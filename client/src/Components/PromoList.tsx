...
export function Promo(props: PromoProps) {
  ...
  return <div className="md:w-[90%] lg:w-[70%] md:m-auto">
    ...
    <div className={`md:flex md:gap-2 ${isLeft? "flex-row":"flex-row-reverse"}`}>
      <div className={`w-full my-4 md:my-0 md:flex ${isLeft? "justify-end":""}`}>
        <img src={props.promo.imgs[0]} className="w-[50%] md:w-[40vw] lg:w-[30vw] m-auto md:m-0 shadow-xs/30 shadow-black" />
      </div>
      <div className="md:flex flex-col text-lg md:text-xl lg:text-2xl">
        <p className={`fraunces-regular mb-2 text-primary-950 ${textAlign}`}>
          <span className="line-through">PHP {props.promo.price}</span>&nbsp;PHP {props.promo.salePrice}
        </p>
        <p className={`fraunces-regular text-md md:text-lg lg:text-xl mb-2 text-primary-950 ${textAlign}`}>
